...
export default function HomePage() {
  ...
  const [editingBooking, setEditingBooking] = useState<MappedBooking | undefined>();
...
instancemixin
  ...
  const loadAllData = async () => {
    try {
      ...
      // Create mapped bookings for components that need camelCase
      const newMappedBookings: MappedBooking[] = bookingsData.map((b: Booking): MappedBooking => ({
        id: b.id,
        name: b.name,
      ...
      setMappedBookings(newMappedBookings);
  ...
  const handleSaveBooking = async (booking: Omit<MappedBooking, "id" | "createdAt" | "updatedAt">) => {
    try {
        ...
        try {
          ...
          if (!existingInvoice) {
            await invoiceService.createInvoice(savedBookingId, {
              clientName: booking.contactName, clientEmail: booking.contactEmail || undefined, clientPhone: booking.contactPhone || undefined,
              eventDateStart: booking.startDate, eventDateEnd: booking.endDate, numberOfGuests: booking.numberOfGuests,
              numberOfRooms: booking.numberOfRooms, basePrice: booking.totalCost, depositAmount: amountPaid,
              balanceDue: balanceDue, totalAmount: booking.totalCost, notes: booking.notes || undefined
            });
  ...
  const handleUpdateBooking = async (booking: MappedBooking) => {
    try {
      await bookingService.updateBooking(booking.id, {
        name: booking.name, booking_type: booking.bookingType, contact_name: booking.contactName,
        contact_email: booking.contactEmail, contact_phone: booking.contactPhone, start_date: booking.startDate,
        end_date: booking.endDate, number_of_guests: booking.numberOfGuests, number_of_rooms: booking.numberOfRooms,
        base_rate: booking.baseRate, per_person_rate: booking.perPersonRate, cleaning_fee: booking.cleaningFee,
        additional_cleaning_fee: booking.additionalCleaningFee, total_cost: booking.totalCost, deposit_amount: booking.depositAmount,
        amount_paid: booking.amountPaid, balance_due: booking.balanceDue, payment_status: booking.paymentStatus,
        confirmed: booking.confirmed, custom_price: booking.customPrice, discount_percent: booking.discountPercent, notes: booking.notes
      });
      await loadAllData();
      setRefreshKey(prev => prev + 1);
    } catch (error) {
  ...
  const handleEditBooking = (booking: MappedBooking) => {
  ...
  const totalGuests = mappedBookings.reduce((sum, b) => sum + b.numberOfGuests, 0);
  const totalRevenue = mappedBookings.reduce((sum, b) => sum + b.totalCost, 0);
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="grid gap-6 md:grid-cols-4 mb-8">
          <Card className="bg-white dark:bg-stone-900 border-stone-200 dark:border-stone-800 hover:shadow-lg transition-all duration-300">
            ...
            <CardContent><div className="text-2xl font-bold text-stone-900 dark:text-stone-100">{mappedBookings.length}</div><p className="text-xs text-stone-600 dark:text-stone-400 mt-1">Active reservations</p></CardContent>
          </Card>
          <Card className="bg-white dark:bg-stone-900 border-stone-200 dark:border-stone-800 hover:shadow-lg transition-all duration-300">
        ...
        <Tabs value={activeTab} onValueChange={(value) => { setActiveTab(value); if (value !== "expenses") setFilteredBookingId(undefined); }} className="space-y-6">
          ...
          <TabsContent value="bookings" className="space-y-4">
            <Card className="bg-white dark:bg-stone-900 border-stone-200 dark:border-stone-800">
              ...
              <CardContent>{mappedBookings.length === 0 ? <div className="text-center py-12 text-stone-500 dark:text-stone-400"><Calendar className="h-12 w-12 mx-auto mb-4 opacity-50" /><p className="text-lg font-medium mb-2">No bookings yet</p><p className="text-sm">Create your first booking to get started</p></div> : <BookingList key={refreshKey} bookings={mappedBookings} onEdit={handleEditBooking} onDelete={handleDeleteBooking} onUpdateBooking={handleUpdateBooking} expenses={expenses} onNavigateToExpenses={handleNavigateToExpenses} />}</CardContent>
            </Card>
          </TabsContent>

          <TabsContent value="calendar" className="space-y-4"><BookingCalendar key={`calendar-${refreshKey}-${mappedBookings.length}`} bookings={mappedBookings} onBookingClick={handleEditBooking} onAddBooking={() => { setEditingBooking(undefined); setBookingDialogOpen(true); }} /></TabsContent>
